...
namespace OverviewerGUI.WinUI.Services
{
    public class NavigationService
    {
        ...
        public void Configure(string key, Type pageType)
        {
            lock (_pages)
            {
                ...
                {
                    throw new ArgumentException("Key is in nav service", key);
                }

                if (_pages.Any(p => p.Value == pageType))
                {
                    throw new ArgumentException("value is in nav service", _pages.First(p => p.Value == pageType).Key);
                }
        ...
        public string GetNameOfRegisteredPage(Type page)
        {
            lock (_pages)
            {
                ...
                {
                    throw new ArgumentException("unknown page", page.Name);
                }
            }
        }
